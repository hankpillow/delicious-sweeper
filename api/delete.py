...
class MainHandler(webapp.RequestHandler):
	...
	def post(self):
		...
		response = {}
		try:
			result = urlfetch.fetch(url=("https://api.del.icio.us/v1/posts/delete?url="+url),headers={"Authorization": "Basic %s" % base64.b64encode(username+":"+password)},deadline=60, allow_truncated=True)
...
def main(has_debug=False):
    application = webapp.WSGIApplication([('/delete', MainHandler)],debug=has_debug)
